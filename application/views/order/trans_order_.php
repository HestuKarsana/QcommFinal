		...
		<div class="panel-body">
			...
			<div class='col-sm-2'>
				<div class="form-group">
									<label class="col-sm-3 control-label">Gunakan alamat profile </label>
									<div class="col-sm-9">
									<div class="input-group">
										<input type="checkbox" id="cekprofile" id="cekprofile" onclick="myCekprofile()">
...
$('#cekongkir').on('click', function (evt) {
	...
	$('#btn_cetak_label').on('click', function (evt) {
			...
			var idorder=pisahlagi[0];
			//var idorder=document.getElementById("idorder");
			
			//alert (idorder);
			$.ajax({
						url: "<?php echo base_url('label_barcode/cetak_labelnya') ?>", 
						type: "post",
	...
	$("#addposting").click(function () {
							var PETUGAS,serviceId,senderName,
								...
								COD = $("#jeniscod").val();
								PETUGAS="<?php $this->session->userdata('ses_id_user');?>";   //'440000024';//htmlspecialchars($this->input->post('username',TRUE),ENT_QUOTES);
							...
							$("#tabeltarif input[name=posradio]:checked").each(function () {
								...
								$.ajax ({ 
										   url: '<?= base_url() ?>index.php/Pickuporder/insOrder',
										   type: "post",
										   dataType: "html",
										   async: true,
										   delay: 250,
										   ...
										   success: function (response) {
												...
												$('#ModalHeader').html('Cetak Label Alamat');
												$('#ModalContent').html(response);
												$('#ModalFooter').html("<button type='button' class='btn btn-primary' data-dismiss='modal' autofocus>Ok</button>");
												//$('#idorder').focus();
												$('#ModalGue').modal('show');
											   */
											    if (response == '') { alert ('Gagal simpan order booking!');$('#addposting').html('Simpan');} else {
...
function myCekprofile() {
  // Get the checkbox
  var checkBox = document.getElementById("cekprofile");
  // Get the output text
